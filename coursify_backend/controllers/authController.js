...
async function registerUser(req, res) {
  ...
  try {
    ...
    const emailExists = await Promise.all([
      pool.query("SELECT * FROM students WHERE email = $1", [trimmedEmail]),
      pool.query("SELECT * FROM instructors WHERE email = $1", [trimmedEmail]),
      pool.query("SELECT * FROM admins WHERE email = $1", [trimmedEmail])
    ]);
    ...
    let query = "";
    let values = [trimmedUsername, trimmedEmail, hashedPassword];

    if (role === "Admin") {
      return res.status(403).json({ message: "Admin registration is not allowed." }); //admin option already removed from frontend , keeping this just for safety
